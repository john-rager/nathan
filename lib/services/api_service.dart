...
class ApiService {
  ...
  Map<String, String> get _headers => {'X-N8N-API-KEY': instance.apiKey, 'Accept': 'application/json'};
  ...
  Future<List<Workflow>> fetchWorkflows() async {
    ...
    throw Exception('Failed to fetch workflows: ${res.statusCode} - ${res.reasonPhrase}');
  }

  Future<List<Execution>> fetchExecutions({String? workflowId}) async {
    final query = <String, String>{};
    if (workflowId != null && workflowId.isNotEmpty) query['workflowId'] = workflowId;
    final res = await http.get(_uri('executions', query), headers: _headers);
    if (res.statusCode == 200) {
      final data = json.decode(res.body);
      final list = (data is List) ? data : (data['data'] ?? []);
      final parsed = List<Execution>.from(list.map((e) => Execution.fromJson(e)));
      parsed.sort((a, b) => b.startedAt.compareTo(a.startedAt));
      return parsed;
    }
    print('Executions API Error: ${res.statusCode} - ${res.body}');
    print('Headers sent: $_headers');
    throw Exception('Failed to fetch executions: ${res.statusCode} - ${res.reasonPhrase}');
  }
}
